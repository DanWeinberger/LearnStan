...
p.vax.grp2 <- pnorm(scale(la3$beta[,2,1] + la3$beta[,2,2]))
p.unvax.grp2 <- pnorm(scale(la3$beta[,2,1]))
...
ar3 <- extract(fit3, permuted = F) # return a list of arrays 
betas <- ar3[,,grep('beta', dimnames(ar3)[[3]])]
```
